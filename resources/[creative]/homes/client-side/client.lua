...
function cRP.entranceHomes(homeName,v,interior,theft)
	...
	if interior == "Middle" then
		...
		table.insert(homes["intern"],{ v[1] + 1.36,v[2] - 14.23,1499.5,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] + 7.15,v[2] - 1.00,1499.0,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] - 0.54,v[2] - 2.46,1499.5,"fridge","Geladeira","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] + 6.54,v[2] + 5.81,1499.0,"wardrobe","Armário","Pressione para abrir." }) 
		end
	elseif interior == "Mansion" then
		...
		table.insert(homes["intern"],{ v[1] - 8.68,v[2] - 3.43,1501.0,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] - 3.97,v[2] - 13.58,1500.5,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] + 5.81,v[2] - 11.88,1500.5,"fridge","Geladeira","Pressione para abrir." })
		end
	elseif interior == "Trailer" then
		...
		table.insert(homes["intern"],{ v[1] - 1.44,v[2] - 2.02,1499.5,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] - 4.36,v[2] - 1.97,1499.2,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] + 0.20,v[2] + 1.70,1499.5,"fridge","Geladeira","Pressione para abrir." })
		end
	elseif interior == "Beach" then
		...
		table.insert(homes["intern"],{ v[1] + 0.11,v[2] - 3.68,1499.5,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] + 8.36,v[2] - 3.60,1499.8,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] - 1.47,v[2] - 0.96,1499.8,"fridge","Geladeira","Pressione para abrir." })
		end
	elseif interior == "Simple" then
		...
		table.insert(homes["intern"],{ v[1] - 4.89,v[2] - 4.15,1501.0,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] + 1.43,v[2] - 2.11,1501.2,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] - 3.33,v[2] + 2.63,1501.2,"fridge","Geladeira","Pressione para abrir." })
		end
	elseif interior == "Motel" then
		...
		table.insert(homes["intern"],{ v[1] + 4.6,v[2] - 6.36,1498.5,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] + 5.08,v[2] + 2.05,1500.3,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] + 4.89,v[2] + 3.40,1500.5,"fridge","Geladeira","Pressione para abrir." })
		end
	elseif interior == "Modern" then
		...
		table.insert(homes["intern"],{ v[1] - 1.63,v[2] - 5.94,1499.7,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] - 0.59,v[2] + 2.95,1499.8,"vault","Baú","Pressione para abrir." })

			if homes["open"] ~= "Modern" then
				table.insert(homes["intern"],{ v[1] + 2.15,v[2] + 7.27,1499.8,"fridge","Geladeira","Pressione para abrir." })
			end
		end
	elseif interior == "Hotel" then
		...
		table.insert(homes["intern"],{ v[1] - 1.69,v[2] - 3.91,1499.8,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] - 2.25,v[2] + 0.95,1499.4,"vault","Baú","Pressione para abrir." })
		end
	elseif interior == "Franklin" then
		...
		table.insert(homes["intern"],{ v[1] - 0.47,v[2] - 5.91,1499.6,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] - 2.60,v[2] - 5.59,1499.3,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] + 4.31,v[2] + 4.58,1499.8,"fridge","Geladeira","Pressione para abrir." })
		end
	elseif interior == "Container" then
		...
		table.insert(homes["intern"],{ v[1] - 1.14,v[2] - 1.38,1500.5,"exit","Sair","Pressione para sair." })

		if not theft then
			table.insert(homes["intern"],{ v[1] + 4.47,v[2] - 1.32,1500.5,"vault","Baú","Pressione para abrir." })
			table.insert(homes["intern"],{ v[1] + 1.45,v[2] - 1.29,1500.5,"fridge","Geladeira","Pressione para abrir." })
		end
	end
		...
		if homes["open"] ~= "" then
			...
			if not IsPedInAnyVehicle(ped) then
				...
				for k,v in pairs(homes["intern"]) do
					...
					if distance <= 1.25 then
						timeDistance = 1
						table.insert(innerTable,{ v[1],v[2],v[3],1.25,"E",v[5],v[6] })
						TriggerEvent("hoverfy:insertTable",innerTable)
						...
						if IsControlJustPressed(1,38) and MumbleIsConnected() then
							...
							elseif v[4] == "wardrobe" then
								...
								exports["dynamic"]:AddButton("Salvar","Salvar suas vestimentas atuais.","props:Consult","save".."-"..homes["open"],false,true)
								exports["dynamic"]:AddButton("Shopping","Abre a loja de vestimentas.","props:Consult","skinshop".."-"..homes["open"],false,true)

								local application,Clothes = vSERVER.propsClothes(homes["open"])
								if application then
									if parseInt(#Clothes) > 0 then
										exports["dynamic"]:SubMenu("Vestir","Abrir lista com todas as vestimentas.","clothesApply")
										exports["dynamic"]:SubMenu("Remover","Abrir lista com todas as vestimentas.","clothesRemove")

										for k,v in pairs(Clothes) do
											exports["dynamic"]:AddButton(v,"Clique para vestir.","props:Consult","apply".."-"..homes["open"].."-"..v,"clothesApply",true)
											exports["dynamic"]:AddButton(v,"Clique para vestir.","props:Consult","remove".."-"..homes["open"].."-"..v,"clothesRemove",true)
										end
									end
								end
...
-- PROPSDYNAMIC
-----------------------------------------------------------------------------------------------------------------------------------------
function cRP.propsDynamic(homeName)
	local consult = vSERVER.returnPropertys(homeName)

	if consult then
		exports["dynamic"]:AddButton("Entrar","Adentrar a propriedade.","props:Consult","enter".."-"..homeName,false,true)
		exports["dynamic"]:AddButton("Credenciais","Reconfigurar os cartões de acesso.","props:Consult","credentials".."-"..homeName,false,true)
		exports["dynamic"]:AddButton("Fechadura","Trancar/destrancar a propriedade.","props:Consult","locks".."-"..homeName,false,true)
		exports["dynamic"]:AddButton("Garagem","Adicionar/reajustar a garagem.","props:Garages",homeName,false,false)
	else
		exports["dynamic"]:AddButton("Entrar","Adentrar a propriedade.","props:Consult","enter".."-"..homeName,false,true)
	end
	exports["dynamic"]:openMenu()
...
-- HOMEGARAGE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("props:Garages")
AddEventHandler("props:Garages",function(homeName)
	...
	local tApp,tCoords,tHeading = vRP.objectCoords("prop_offroad_tyres02")
	homeCoords[homeName] = { x = mathLegth(tCoords["x"]), y = mathLegth(tCoords["y"]), z = mathLegth(coords["z"]) }
	
	if tApp then 
		local cApp,cCoords,cHeading = vRP.objectCoords("zentorno")
		TriggerEvent("Notify","verde","Garagem adicionada.",10000)
		homeCoords[homeName]["1"] = { mathLegth(cCoords["x"]),mathLegth(cCoords["y"]),mathLegth(cCoords["z"]),mathLegth(cHeading) }
